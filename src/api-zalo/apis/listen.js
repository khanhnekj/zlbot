...
import { decodeEventData, getGroupEventType, getFriendEventType, logger } from "../utils.js";
...
export class Listener extends EventEmitter {
  ...
  start() {
    ...
    this.ws.onclose = () => {
      ...
      this.emit("closed");
      if (!this._manualStop) {
        logger.error("[WS] Bị ngắt! Reconnect sau 5 giây...");
        setTimeout(() => { if (!this._manualStop) this.start(); }, 5000);
      }
    };
  ...
  async handleControlEvents(parsed) {
    ...
    for (const control of controls) {
      if (control.content.act_type == "file_done") {
        ...
        const groupEvent = initializeGroupEvent(groupEventData, getGroupEventType(act), act);
        if (groupEvent.isSelf && !this.selfListen) continue;
        this.emit("group_event", groupEvent);
      } else if (control.content.act_type == "friend") {
        const friendEventData = typeof control.content.data == "string" ? JSONBig.parse(control.content.data) : control.content.data;
        const act = control.content.act;
        const friendEvent = initializeFriendEvent(friendEventData, getFriendEventType(act));
