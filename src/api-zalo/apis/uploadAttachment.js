...
import { encodeAES, getFileSize, getImageMetaData, getMd5LargeFileObject, handleZaloResponse, makeURL, request, logger } from "../utils.js";
import { checkConfigUploadAttachment, getProphylacticUploadAttachment, setProphylacticUploadAttachment } from "../../index.js";
import { uploadTempFile } from "../../utils/core/util.js";
    ...
    for (const filePath of filePaths) {
      ...
      if (getProphylacticUploadAttachment() || isUseProphylactic) {
        try {
          ...
          logger.error("Upload Attachment API Thứ 3 Đã Xảy Ra Lỗi, Vui Lòng Kiểm Tra Lại!");
          logger.error("Chuyển sang dùng api chính từ zalo (Không khả thi khi upload nhiều file trong thời gian ngắn)!");
        }
      }
      ...
      try {
        ...
        logger.error("Lỗi upload lần 1, thử lại sau 1s:", error);
